...
class List:
    ...
    def insert(self, i, value):
        ...
        if i <= 0:
            ...
            node = _List_Node(value, first._next)
            first._next = node
    ...
    def _reverse_r_aux(self, previous, current):
        if current is None:
            ...
            self._reverse_r_aux(current, temp)
        return
    ...
    def split_alt_r(self):
        ...
        even = List()
        odd = List()
        self._split_alt_r_aux(even, odd, 0)
        return even, odd

    def _split_alt_r_aux(self, even, odd, index):
        """
        A recursive helper method for split_alt_r. This method removes the front
        node from the self list and appends it to either the even or odd list
        ...
        if self._front is not None:
            ...
            if index % 2 == 0:
                if even._front is None:
                    even._front = even._rear = node
                else:
                    even._rear._next = node
                    even._rear = node
            else:
                if odd._front is None:
                    odd._front = odd._rear = node
                else:
                    odd._rear._next = node
                    odd._rear = node

            even._count += index % 2 == 0
            odd._count += index % 2 != 0

            self._count -= 1
            if self._front is None:
                self._rear = None

            self._split_alt_r_aux(even, odd, index + 1)
